...
// 7) -------------------------------------------------------
let number = [76,35,73,13,4,88,23,12,86,1,54,9,10,2]
// remember numbers could be reassigned, added to, etc.
let newNumbersA = number.filter((num) => {
  return num < 5
})
...
function getNumbersSquared(numbers) {
  for (let i = 0; i < numbers.length; i++) {
    numbers[i] = numbers[i] ** 2; // Squaring each number and updating the original array
  }
  return numbers;
}
